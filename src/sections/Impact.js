...
const stories = [
    { label: 'Education Outreach', color: '#E8392A' },
    { label: 'Community Events', color: '#C62D1F' },
    { label: 'Health Workshops', color: '#1A1A2E' },
    { label: 'Family Support', color: '#2d1f3d' },
    { label: 'Women Empowerment', color: '#E8392A' },
    { label: 'Youth Mentorship', color: '#C62D1F' }
];
...
export function mountImpact(element) {
    ...
    <div class="impact-container container">
      ...
      <div class="impact-gallery reveal reveal-delay-2">
        ${stories.map((story, i) => `
          <div class="gallery-item gallery-item--${i + 1}" style="--item-color: ${story.color}">
            <div class="gallery-overlay">
              <span class="gallery-label">${story.label}</span>
            </div>
          </div>
        `).join('')}
      </div>
